...
class VulnersAnalyzer(Analyzer):
    ...
    def summary(self, raw):
        ...
        if self.service == 'vulnerability':
            predicate = "CVE"
            # Se usa get con un valor por defecto para evitar KeyError
            vulners_score = raw.get('vulners_AI', 'No score')
            if not raw['exploits']:
                level = 'suspicious'
                value = f"CVSS score: {raw['cvss']['score']} / Vulners score: {vulners_score} / No exploits"
            else:
                level = 'malicious'
                value = f"CVSS score: {raw['cvss']['score']} / Vulners score: {vulners_score} / Exploits {len(raw['exploits'])}"
    ...
    def run(self):
        ...
        if self.service == 'vulnerability':
            if self.data_type == 'cve':
                ...
                if cve_info:
                    # Usa get para 'enchantments' y 'vulnersScore', con valor por defecto
                    vulners_score = cve_info.get('enchantments', {}).get('vulnersScore', 'No score')
                    full_cve_info = {
                        'service': self.service,
